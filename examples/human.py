...
def userConsole(args):
    ...
    env = ScienceWorldEnv("", args['jar_path'],
    ...
    env.load(taskName, args['var_num'],
             simplificationStr, generateGoldPath=True)
    ...
    print("Possible actions (with IDs): " +
          str(env.get_possible_actions_with_IDs()))
    print("Possible object types: " + str(env.get_object_types()))
    print("Object IDX to Object Referent LUT: " + str(lut))
    print("\n")
    print("Possible object referents LUT: " +
          str(env.get_possible_object_referent_LUT()))
    print("\n")
    ...
    print("Variation: " + str(args['var_num']) +
          " / " + str(env.get_max_variations(taskName)))
    ...
    while (userInputStr not in exitCommands):
        ...
        if prompt_toolkit_available:
            actions_completer = WordCompleter(
                valid_actions, ignore_case=True, sentence=True)
