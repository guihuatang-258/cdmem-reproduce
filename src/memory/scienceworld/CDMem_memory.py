...
class ShortMemory:
    ...
    def add(self, label: str, value: str) -> None:
        ...
        if label == 'look':
            ...
            self.history = [
                item for item in self.history if item['label'] != 'look']
    ...
    def recent_actions(self) -> List[str]:
        actions = [item['value']
                   for item in self.history if item['label'] == 'action']
        return actions[-3:]
...
class GlobalMemory:
    ...
    def short2long(self, expert_trajectory, env_idx, trial_idx):
        ...
        if env_curiocity or len(self.env_memory[env_description]['increment_traj']) > self.env_bs:
            ...
            increment_known_obs = [sample['function']
                                   for sample in samples if sample['function'] != 'None']
        ...
        if task_description:
            ...
            if task_type in self.task_memory:
                ...
                if 'fail' in self.task_memory[task_type]:
                    split_summary = self._split_summary(
                        self.task_memory[task_type]['fail']['action_guidance'])
                    if self.is_vector:
                        repeat_scores = []
